...
public sealed class AuthController : ControllerBase
{
    ...
    [ProducesResponseType<TokenResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    public async Task<ActionResult<TokenResponse>> RefreshAsync(CancellationToken cancellationToken)
    {
        Guid userId = Guid.Parse(User.FindFirstValue("sub")!);
        IssuedToken token = await _auth.RefreshAsync(userId, cancellationToken);
        return Ok(new TokenResponse(token.AccessToken, "Bearer", token.ExpiresInSeconds));
    }
}
